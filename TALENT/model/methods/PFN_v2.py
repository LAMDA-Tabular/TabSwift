...
class TabPFNMethod(Method):
    ...
    def construct_model(self, model_config = None,cat_indices=[]):
        if self.is_regression:
            ...
            self.model = TabPFNRegressor(device = self.args.device,random_state = self.args.seed,n_estimators=16,ignore_pretraining_limits=True,categorical_features_indices=cat_indices)
        else:
            from TALENT.model.models.PFN_v2 import TabPFNClassifier
            self.model = TabPFNClassifier(device = self.args.device,random_state = self.args.seed,n_estimators=16,ignore_pretraining_limits=True,categorical_features_indices=cat_indices)  
