...
class TabSwiftMethod(Method):
    ...
    def construct_model(self, model_config = None,cat_indices=[]):
        if self.is_regression:
            ...
            self.model = TabSwiftRegressor(device = self.args.device,
                ...
                class_shift=False,                 # whether to apply cyclic shifts to class labels
                ...
                batch_size=16,                     # process this many ensemble members together (reduce RAM usage)
                use_amp=True,                     # use automatic mixed precision for faster inference
                model_path = self.args.pretrain_model_path, # where the model checkpoint is stored
            ...
            # print(self.args.pretrain_model_path)
    ...
    def predict(self, data, info, model_name):
        ...
        print('Predict time: {:.4f}s'.format(self.predict_time))
